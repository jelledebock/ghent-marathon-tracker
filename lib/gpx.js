...
var find_in_course = function(location, index){
    ...
    //Fine tuning step : find if next point isn't more accurate (smaller than
    //  desired proximity).
    if(i+1<parcours.points.length){
        while(i+1<parcours.points.length && last_proximity>haversine({'latitude':location[0], 'longitude': location[1]},{'latitude':parcours.points[i+1][0], 'longitude': parcours.points[i+1][1]}, {unit: 'meter'})){
    ...
    if(i==parcours.points.length){
        ...
        return [parcours.points[i], parcours.cum_distance[i], i, last_proximity];
    }
};
